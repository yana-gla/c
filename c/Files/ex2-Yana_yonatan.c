...
typedef struct{
	char *cmmnd;
	int (*cmp_fnc)(const char *cmmnd, const char *input); // ממליץ לעשות טייפ-דפ
	int (*op_func)(const char *filename, const char *input); // כנל
}str_op_t;
...
/*Remove Function*/
int RmvFnct(const char *filename, const char *input) // את מוחקת את הקובץ, לא את הפונקציה. צריך לשנות את השם של הפונקציה שלך שיהיה יותר ברור
{
	(void) input;
...
/*Exit Function*/
int ExtFnct(const char *filename, const char *input) // הייתי אומר exit program
{
	(void) filename;
	(void) input;
	return (EXIT);
...
/*Count Function*/
int CntFnct(const char *filename, const char *input) // כנל - לדייק את שם הפונקציה - היא סופרת שורות בקובץ
{
	size_t num_lns = 0;
	FILE* fp = fopen(filename, "r");
	char *tmp = (char*)calloc(MAX_LIMIT, sizeof(char));
	if (0 == tmp) return -1; // לדעתי צריך להוריד כאן שורה אחרי הסוגריים, אבל לא מאוד קריטי
	
	(void) input;
	
	while (NULL != fgets(tmp, MAX_LIMIT, fp)) // אני השתמשתי בfgetc(fp) != EOF , לא בטוח מה יותר נכון
	{
	    ++num_lns;
	}
...
int ApndFnct(const char *file_name,const  char *str_input)
{
	...
	{ 
		...
		return -1; // לא יודע אם יש צורך להחזיר כאן, לדעתי מספיק לכתוב למשתמש שיש בעיה בפתיחת הקובץ
	}
